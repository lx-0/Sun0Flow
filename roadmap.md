...
| 2026-05-15 | Error backend = GlitchTip (errors.yester.cloud), not Sentry SaaS | Self-hosted, Sentry-protocol-compatible for error + performance envelopes. Replay envelopes rejected — `replayIntegration()` removed from `sentry.client.config.ts`. Trace sampling kept at 10%. |
...
| 2026-05-15 | GlitchTip MCP integration kept private (yesterday-private-plugins) | Bundled as `glitchtip-mcp` skill inside the private `yesterday-systems-operations` plugin alongside an auto-starting HTTP MCP server. Yesterday-internal (errors.yester.cloud). |
